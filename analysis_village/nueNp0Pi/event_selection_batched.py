...
from analysis_village.nueNp0Pi.dataset_locations import (
...
def survey_files(
    ...
    for sample in samples:
        ...
        print(f"[survey_files] {sample}: looking in {EVENT_SELECTION_GLOBS[sample]}", flush=True)
        paths = list(iter_event_selection_df_paths(sample))
        print(f"[survey_files] {sample}: found {len(paths)} file(s)", flush=True)
