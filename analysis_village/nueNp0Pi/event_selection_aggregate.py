...
from analysis_village.nueNp0Pi.event_selection_pipeline_def import (
    build_pipeline, EFFICIENCY_VARS,
)
from analysis_village.nueNp0Pi.selection_framework import (
    BarBreakdown,
...
def render_overlay_plots(
    ...
    from analysis_village.nueNp0Pi.selection_framework import ChunkRunner
    ...
    for var_save_name in sorted(vars_seen):
        ...
        for stage_key in stage_keys:
            ...
            ax_eff.errorbar(bin_centers, this_eff, yerr=this_eff_err,
                            fmt="none", color=color, label=label, markersize=4, zorder=4,
                            capsize=3)
...
def render_cutflow_table(merged: dict, save_fig_dir: str, pot_str: str,
    ...
    if save_fig:
        plt.savefig(path.join(save_fig_dir, f"cutflow_table{fig_ext}"),
...
def render_breakdown_table(merged: dict, save_fig_dir: str, pot_str: str,
    ...
    if save_fig:
        plt.savefig(path.join(save_fig_dir, f"bkgd_breakdown_table{fig_ext}"),
                    dpi=dpi, bbox_inches="tight")
    if show_fig:
        plt.show()
    else:
