"""Memory-efficient, chunked event-selection/histogram-accumulation framework.

This module is analysis-agnostic: it knows nothing about any particular signal
definition or event-category breakdown. It turns an all-in-memory
event-selection notebook into a chunked pipeline that
...
A second pass reads back the per-chunk pickles, sums intrinsic-weight
histograms across chunks (``aggregate_chunk_files``, ``merge_samples``),
applies **global** POT / gates factors from summed chunk metadata
(``apply_global_exposure_scales``), and then renders the final plots.

Memory note
-----------
This design only ever holds one file's events in RAM at a time, plus the
(negligible) accumulated histograms. This is what makes it run on a full
sample.

Making it work for a specific analysis
---------------------------------------
Two things are supplied by the caller (an analysis under ``analysis_village``),
not by this module:

* ``breakdown_registry`` -- a ``{breakdown_type: (n_categories, get_cuts_fn)}``
  dict, where ``get_cuts_fn(df, ret_cuts=True)`` returns a list of boolean
  masks (one per category, in stacking order) for a given event dataframe.
  Analyses typically build this from their own ``categories.py``.
* ``signal_mask_fn`` -- a ``df -> boolean mask`` callable giving this
...
----------------------------------
* A new selection cut    -> add a new ``Stage``.
...
# Consumer-side systematic bands are typically loaded from pre-saved files
# elsewhere (e.g. ``pyanalib.syst_disk_layout`` consumers); this is for
# building chunked syst covariances at the producer stage.
# ---------------------------------------------------------------------------
...
class OverlayHistData:
    ...
    ``n_cat`` is the number of stacking categories for this ``breakdown_type``
    (the caller resolves it from its own ``breakdown_registry``). Histograms in
    ``mc_hist`` and ``mc_err2`` are indexed [category, bin] in whatever order
    the caller's ``get_cuts_fn(df, ret_cuts=True)`` returns cuts, so downstream
    plotting code must use the same order consistently.
    ...
    def fill_from_df(
        ...
        var_col,
        sample: str,
        get_cuts_fn: Optional[Callable] = None,
        mc_univ_syst_tags: Optional[Tuple[str, ...]] = None,
    ):
        """Fill from one chunk of one sample. ``sample`` is mc/intime/dirt/data/offbeam.

        Uses ``df.pot_weight`` (already attached upstream) unless omitted.

        ``get_cuts_fn``: required when ``sample=='mc'`` -- a callable
        ``get_cuts_fn(df, ret_cuts=True) -> list[bool mask]`` giving this
        histogram's stacking categories, in the same order as ``n_cat``.
        ...
        build a fractional covariance across universes.
        """
        if df is None:
            return
        ...
        if sample == "mc":
            if get_cuts_fn is None:
                raise ValueError("get_cuts_fn is required when sample=='mc'")
...
class BarBreakdown:
    ...
    Stores POT-weighted event counts per category (for whatever
    ``breakdown_type`` this instance was created with) for MC and plain
    POT-weighted counts for intime/dirt. Per-stage and per-breakdown.
    """
    breakdown_type: str
    ...
    @classmethod
    def empty(cls, breakdown_type: str, n_cat: int) -> "BarBreakdown":
        return cls(breakdown_type=breakdown_type, mc_counts=np.zeros(n_cat))

    def fill_mc(self, df: pd.DataFrame, get_cuts_fn: Callable):
        if df is None or len(df) == 0:
            return
...
class EfficiencyAccumulator:
    ...
    * **Numerator (selected truth)** — histogram of reconstructed signal slices on
      ...
      (available from the first stage for the primary-lepton/hadron kinematics),
      otherwise fall back to ``VariableConfig.var_evt_truth_col`` (matched-track
      truth, available after track ID).
    ...
    var_save_name: str
    bins: np.ndarray
    # Selected signal in FV: evt × var_evt_truth_col (per stage)
    ...
    def fill_denominator_from_mcnu(self, mcnu_df, var_config, signal_mask_fn: Callable):
        """Incremental truth histogram on generated neutrinos (call once/chunk/stage-loop).

        ``signal_mask_fn``: ``df -> boolean mask`` giving the analysis's truth-signal
        definition (e.g. ``lambda df: IsNuInFV_NumuCC_1p0pi(df, detector=DETECTOR)``).
        """
        ...
        weights = np.asarray(weights, dtype=float)
        sig_mask = signal_mask_fn(mcnu_df)
...
class PlotSpec:
    ...
    should NOT propagate to subsequent stages.
    """
    var_config: Any                  # VariableConfig
    breakdown_type: str              # analysis-defined key into breakdown_registry
    selector: Callable[[Dict[str, pd.DataFrame], str], Optional[pd.DataFrame]]
    name_suffix: str = ""            # for save_name disambiguation
    save_kwargs: Dict[str, Any] = field(default_factory=dict)  # kwargs for a plotting helper
...
class ChunkRunner:
    ...
    are aggregated by ``aggregate_chunk_files`` -- which simply calls ``+=``.

    ``breakdown_registry`` and ``signal_mask_fn`` are supplied by the calling
    analysis (see module docstring); this class has no built-in physics.
    """
    ...
    def __init__(
        ...
        self.breakdown_registry = breakdown_registry
        ...
        # bar plots (per-stage breakdown counts), one dict per bar_breakdown_types entry
    ...
    def _fill_plot(self, stage_key: str, ps: PlotSpec, state: Dict[str, pd.DataFrame]):
        ...
        if df is None:
            return
        n_cat, get_cuts_fn = self.breakdown_registry[ps.breakdown_type]
    ...
    def _fill_breakdown(self, stage_key: str, state: Dict[str, pd.DataFrame]):
        if stage_key not in self.bar:
            self.bar[stage_key] = {
                bt: BarBreakdown.empty(bt, self.breakdown_registry[bt][0])
                for bt in self.bar_breakdown_types
            }
        evt = state.get("evt")
        if self.sample == "mc":
            for bt in self.bar_breakdown_types:
                get_cuts_fn = self.breakdown_registry[bt][1]
                self.bar[stage_key][bt].fill_mc(evt, get_cuts_fn)
