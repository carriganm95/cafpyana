...
def get_n_split(file, reference_key: str = "evt") -> int:
    """Return the number of splits available for ``reference_key`` in ``file``.

    Reads the ``split`` table for the nominal value, then verifies against
    the keys actually present in the HDF store so a stale ``n_split`` entry
    never causes a KeyError downstream.
    """
    try:
        nominal = int(pd.read_hdf(file, key="split").n_split.iloc[0])
    except Exception:
        nominal = None

    prefix = f"{reference_key}_"
    with pd.HDFStore(file, mode="r") as store:
        actual = sum(
            1 for k in store.keys()
            if k.strip("/").startswith(prefix)
            and k.strip("/")[len(prefix):].isdigit()
        )
    ...
    if actual != nominal:
        logger.warning(
            f"[get_n_split] {file}: split table says n_split={nominal} "
            f"but only {actual} '{reference_key}_*' keys exist; using {actual}"
        )
    return actual
...
def load_dfs(file, keys2load, n_max_concat=100):
    ...
    logger.debug(f"[{Path(__file__).name}]: Loading {n_concat} splits from {file} for keys {keys2load}")
